...
impl GameState for PlayerInputState {
    ...
    fn update(&mut self, delta_time: f32, black_board: &mut Blackboard) -> Option<GameStateIndex> {
        if self.transition_to_game_over {
            ...
            if !self.animator.is_animating() {
                black_board.game_board.apply_move_on_column(self.slot_picked.unwrap(),false);
        ...
        if let Some(slot_choice) = self.slot_picked {
            ...
            if !game_over {return Some(GameStateIndex::ComputerExecutionState);}
    ...
    fn mouse_click(&mut self, position: [f32; 2]) {
        if self.slot_picked.is_some() { return;}
